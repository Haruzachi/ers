...
<head>
    ...
    <title>Barangay Emergency Logging Management</title>
...
<div class="menu-section">
    ...
    <div class="menu-items">
        ...
        <div class="menu-item active" onclick="toggleSubmenu('fire-incident')">
        ...
        <div id="fire-incident" class="submenu">
            <a href="" class="submenu-item active">Logging</a>
...
/* Map full screen */
#map {
    position: relative; /* relative or absolute works */
    z-index: 0;         /* lowest possible */
}
...
<!-- Map container -->
<div id="map" style="height: 100vh; width: 100%; border-radius: 0.5rem;"></div>bb
<!-- Leaflet JS -->
...
<script src="https://unpkg.com/leaflet/dist/leaflet.js"></script>
     ...
     <!-- JS Notification and Map Scripts -->
        <script>
        // Initialize map
        let map = L.map('map').setView([14.091, 121.178], 13);
        ...
        function goToNotification(lat, lng, message) {
            markers.forEach(m => map.removeLayer(m));
            markers = [];
            const marker = L.marker([lat, lng]).addTo(map).bindPopup(message).openPopup();
            markers.push(marker);
        ...
        function renderNotifications() {
            ...
            notifications.forEach(notif => {
                ...
                li.dataset.lat = notif.lat;
                li.dataset.lng = notif.lng;
                li.addEventListener('click', function() {
                    goToNotification(parseFloat(this.dataset.lat), parseFloat(this.dataset.lng), this.textContent);
    ...
    // Add fresh markers
    notifications.forEach(notif => {
        const marker = L.marker([notif.lat, notif.lng])
            .addTo(map)
            .bindPopup(notif.message);
        ...
        fetch('logging.php?action=fetch_notifications')
    .then(res => res.json())
    .then(data => {
        ...
        // Auto focus on latest new notification
        if (notifications.length > 0 && notifications.length !== oldCount) {
            const latest = notifications[0];
            goToNotification(latest.lat, latest.lng, latest.message);
        }
    })
