...
$full_name = $user ? trim($user['first_name'].' '.$user['middle_name'].' '.$user['last_name']) : "User";
$role = $user ? $user['role'] : "USER";

// Fetch notifications with severity from database
$stmt = $pdo->query("SELECT id, message, severity, created_at FROM emergency_notifications ORDER BY created_at DESC");
$notifications = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
.severity.high { background-color: #e53935; }
.severity.medium { background-color: #fb8c00; }
.severity.low { background-color: #43a047; }
/* Notification panel */
.notif-badge { background: red; color: white; font-size: 12px; padding: 2px 6px; border-radius: 50%; position: absolute; top: -5px; right: -5px; }
.notifications-panel { position: fixed; top: 60px; right: 20px; width: 300px; max-height: 400px; overflow-y: auto; background-color: white; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.2); display: none; z-index:9999; }
.notifications-panel .notifications-list { list-style:none; padding:0; margin:0; }
.notifications-panel .notifications-list li { padding:10px; border-bottom:1px solid #eee; cursor:pointer; }
.notifications-panel .notifications-list li:hover { background:#f0f0f0; }
</style>
</head>
...
<div class="menu-section">
    ...
    <div class="menu-items">
        ...
        <div class="menu-item active" onclick="toggleSubmenu('volunteer')">
        ...
        <div id="volunteer" class="submenu">
            <a href="" class="submenu-item active">Severity</a>
        </div>
    ...
    <table>
        ...
        <tbody id="severityTable">
            ...
            <tr>
                ...
                <td>
                    <span class="severity <?= strtolower($notif['severity']) ?>"><?= htmlspecialchars($notif['severity']) ?></span>
                </td>
...
function fetchNotifications() {
    fetch('Severity.php?action=fetch_notifications')
    .then(res => res.json())
    .then(data => {
